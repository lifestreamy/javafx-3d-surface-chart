...
public class DelaunayTriangulation {
    ...
    private final Vector<Triangle> addedTriangles;
    ...
    private int modCount, modCount2;
     ...
     */
    private static Triangle findnext2(Triangle v) {
    ...
    public void deletePoint(Point pointToDelete) {
        ...
        Vector<Point> pointsVec = findConnectedVertices(pointToDelete);
        if (pointsVec == null) {
            return;
        }
        ...
        while (pointsVec.size() >= 3) {
            ...
            Point p = findDiagonal(Objects.requireNonNull(triangle), pointToDelete);
    ...
    public Point[] calcVoronoiCell(Triangle triangle, Point p) {
        ...
        // which is the perpendicular bisector of the half plane line
            ...
            if (!triangle.getAbTriangle().isHalfplane()) {
                neighbor = triangle.getAbTriangle();
            }
            else if (!triangle.getBcTriangle().isHalfplane()) {
                neighbor = triangle.getBcTriangle();
            }
            else if (triangle.getBcTriangle().isHalfplane()) {
                neighbor = triangle.getCaTriangle();
            }
            ...
            if (!Objects.requireNonNull(neighbor).getA().equals(triangle.getA()) && !neighbor.getA()
                                                                                             .equals(triangle.getB()))
                third = neighbor.getA();
            if (!neighbor.getB().equals(triangle.getA()) && !neighbor.getB().equals(triangle.getB()))
                third = neighbor.getB();
            if (!neighbor.getC().equals(triangle.getA()) && !neighbor.getC().equals(triangle.getB()))
                third = neighbor.getC();

            // delta (slope) of half plane edge
            double halfplaneDelta = (triangle.getA().getY() - triangle.getB().getY())
            ...
            double yOrient =
                    halfplaneDelta * (Objects.requireNonNull(third).getX() - triangle.getA().getX()) + triangle.getA()
                                                                                                               .getY();
            boolean above = true;
    ...
    private Vector<Point> findConnectedVertices(Point point) {
        ...
        if (!triangle.isCorner(point)) {
            System.err
                    .println("findConnectedVertices: Could not find connected vertices since the first found triangle" +
                            " doesn't"
    ...
    public Vector<Triangle> findTriangleNeighborhood(Triangle firstTriangle, Point point) {
        Vector<Triangle> localTriangles = new Vector<>(30);
