 ...
 * Therefore, one can easily locate a triangle in close proximity of the required
 ...
 * roughly recurring the searched triangles by square(xCellCount * yCellCount), as
 * only the triangles inside the cell are searched.
 * <p>
 * The index takes xCellCount * yCellCount capacity. While more cells allow
 * faster searches, even a small grid is helpful.
 * <p>
 ...
 * (only refining existing triangles) this is never an issue in real life.
 */
public class GridIndex {
     ...
     * The smaller the cells the less triangles that fall in them, which means
     ...
     * whole index with the enlarged region.
     *
     * @param updatedTriangles changed triangles of the triangulation. This may be added
    ...
    private void updateCellValues(int startXCell, int startYCell, int lastXCell, int lastYCell,
        ...
        for (int i = startXCell; i <= lastXCell; i++) {
            // Find a triangle at the beginning of the current column
    ...
    /**
     * Finds a valid (existing) triangle adjacent to a given invalid cell
     *
