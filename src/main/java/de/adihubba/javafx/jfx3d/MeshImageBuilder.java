...
public class MeshImageBuilder {
    ...
    public static double calculateZValueInTriangle(Point2D p, Point3D p0, Point3D p1, Point3D p2) {
        double divider =
                ((p1.getX() - p0.getX()) * (p2.getY() - p0.getY())) - ((p2.getX() - p0.getX()) * (p1.getY() - p0
                        .getY()));
        return p0.getZ() + (((((p1.getX() - p0.getX()) * (p2.getZ() - p0.getZ())) - ((p2.getX() - p0.getX()) * (p1
                .getZ() - p0.getZ()))) / divider) * (p.getY() - p0.getY()))
                - (((((p1.getY() - p0.getY()) * (p2.getZ() - p0.getZ())) - ((p2.getY() - p0.getY()) * (p1.getZ() - p0
                .getZ()))) / divider) * (p.getX() - p0.getX()));
    }
    ...
    public static double calculateYValueInTriangle(Point2D p, Point3D p0, Point3D p1, Point3D p2) {
        double divider =
                ((p1.getZ() - p0.getZ()) * (p2.getX() - p0.getX())) - ((p1.getX() - p0.getX()) * (p2.getZ() - p0
                        .getZ()));
        return p0.getY() + (((((p1.getY() - p0.getY()) * (p2.getX() - p0.getX())) - ((p1.getX() - p0.getX()) * (p2
                .getY() - p0.getY()))) / divider) * (p.getY() - p0.getZ()))
                - (((((p1.getY() - p0.getY()) * (p2.getZ() - p0.getZ())) - ((p2.getY() - p0.getY()) * (p1.getZ() - p0
                .getZ()))) / divider) * (p.getX() - p0.getX()));
    }
    ...
    public static boolean isPointInTriangle(Point2D p, Point3D p0, Point3D p1, Point3D p2) {
        double area = 0.5 * (-p1.getZ() * p2.getX() + p0.getZ() * (-p1.getX() + p2.getX()) + p0.getX() * (p1.getZ() - p2
                .getZ()) + p1.getX() * p2.getZ());
        int sign = area < 0.0 ? -1 : 1;
        double s = (p0.getZ() * p2.getX() - p0.getX() * p2.getZ() + (p2.getZ() - p0.getZ()) * p.getX() + (p0.getX() - p2
                .getX()) * p.getY()) * sign;
        double t = (p0.getX() * p1.getZ() - p0.getZ() * p1.getX() + (p0.getZ() - p1.getZ()) * p.getX() + (p1.getX() - p0
                .getX()) * p.getY()) * sign;
    ...
    public Image createImage(MeshCalculationComposite calculationObject) {
        ...
        IntStream.range(0, imageSize).forEach(x -> IntStream.range(0, imageSize).parallel().forEach(y -> pw
                .setColor(x, y, calculateColor(0, x, y, factor, calculationObject))));

        // save image for testing purposes
        //        try {
        //            ImageIO.write(SwingFXUtils.fromFXImage(img, null), "png", new File(System.getProperty("user
        //            .home") + "\\Desktop\\Vola3DImage.png"));
    ...
    private Color calculateColor(int recursionDepth, int x, int y, double factor,
                                 MeshCalculationComposite calculationObject) {
        if (recursionDepth >= 2) {
            return transparentColor;
        }
        final Point2D point2Calculate = new Point2D(x * factor, y * factor);

        Optional<Triangle3D> triangleMatch = calculationObject.getTriangle3DList().stream().filter(t -> MeshImageBuilder
                .isPointInTriangle(point2Calculate, t.getP0(), t.getP1(), t.getP2())).findFirst();
        if (triangleMatch.isPresent()) {
            Triangle3D triangle = triangleMatch.get();
            double calculatedYValue = MeshImageBuilder
                    .calculateYValueInTriangle(point2Calculate, triangle.getP0(), triangle.getP1(), triangle.getP2());
    ...
    private Color getColor(double value, MeshCalculationComposite calculationObject) {
        java.awt.Color hsb =
                java.awt.Color.getHSBColor((float) (value / calculationObject.getSize() * (23.0 / 36.0)), 1.0f, 1.0f);
